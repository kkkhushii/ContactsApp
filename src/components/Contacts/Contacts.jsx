...
import { useContact } from '../../components/contextApi/ContactContext';
      ...
      <div className="p-3 border-bottom d-flex ">
        <form className="flex-grow-1">
          <input
            ...
            type="text"
            className="form-control mb-1 mt-1 form-control"
            value={serach}
      ...
      {getContacts.length === 0 ? (
        ...
        getContacts.map((contact, index) => (
          <ul className="nav cursor-pointer" key={index}>
            <li
              className={`w-100 bg-white nav-item ${selectedContact && selectedContact.id === contact.id ? 'selected-contact' : 'bg-dark'}`}
            >
              <div className="d-flex align-items-center justify-content-between p-3 mb-1">
                ...
                >
                  <div>
                    <img src={contact.profile} alt="user" className="rounded-circle" width="50" />
                  </div>
                  <div className="mx-2 flex-grow-1">
                    <h5 className="mb-0 text-truncate" style={{ width: '140px', fontSize: '1rem' }}>
                      {contact.firstName} {contact.lastName}
                    </h5>
                    <small style={{ fontSize: '0.875' }}>{contact.dapartment}</small>
                  </div>
                </div>
